...
namespace {
  ...
  Value value_draw(SFThread* thisThread) {
  ...
  struct ThreadHolding {
    explicit ThreadHolding(SFThread* thisThread, Key posKey, int ply) {
       ...
       if (location)
       {
          ...
          SFThread* tmp = (*location).thread.load(std::memory_order_relaxed);
          if (tmp == nullptr)
          {
...
void MainThread::search() {
  ...
  else
  {
      ...
      {
          for (SFThread* th : Threads)
              std::swap(th->rootMoves[0], *std::find(th->rootMoves.begin(), th->rootMoves.end(), bookMove));
      }
      else
      {
          Threads.start_searching(); // start non-main threads
          SFThread::search();          // main thread start searching
      }
  }
...
/// SFThread::search() is the main iterative deepening loop. It calls search()
/// repeatedly with increasing depth until the allocated thinking time has been
/// consumed, the user stops the search, or the maximum search depth is reached.

void SFThread::search() {
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    SFThread* thisThread = pos.this_thread();
...
string UCI::pv(const Position& pos, Depth depth, Value alpha, Value beta) {
  ...
  for (size_t i = 0; i < multiPV; ++i)
  {
      ...
      // CN
      if(pos.this_thread()->pvCallback) {
        pos.this_thread()->pvCallback(rootMoves[i].pv[0], d, rootMoves[i].selDepth, v);
      }
  }
